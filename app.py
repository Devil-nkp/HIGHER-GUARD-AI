...
# Custom BERT layer for model compatibility
...
def load_models():
    ...
    try:
        ...
        if os.path.exists(Config.MODEL_PATH):
            ...
            logger.warning("⚠️ Fraud detection model not found. Running in simulation mode.")
...
def analyze_job_posting():
    """API endpoint to analyze a job posting image"""
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            # Check if the extracted text resembles a job posting
            ...
            # Analyze the text for fraud indicators
            logger.info("Starting fraud analysis")
            if fraud_model and tokenizer:
                # Use the actual AI model if available
...
def analyze_text_only():
    """API endpoint to analyze job posting text directly"""
    try:
        ...
        # Check if the text resembles a job posting
...
def submit_feedback():
    """API endpoint to collect user feedback on analysis results"""
    try:
        data = request.get_json()
